...
function clientCode1() {
  ...
  if (original.component === copy.component) {
    ...
    console.log(
      'Simple component has been cloned, different object reference.',
    );
  }
  if (original.circularReference === copy.circularReference) {
    ...
    console.log(
      'Component with back reference has been cloned, different object reference.',
    );
  }
  if (
    original.circularReference.prototype === copy.circularReference.prototype
  ) {
    console.log('Component with back reference is linked to original object.');
  } else {
    console.log(
      'Component with back reference is linked to the clone, different object reference.',
    );
  }
}
...
function clientCode2() {
  ...
  coachOriginalPrototype.preference = new CoachPreference(
    coachOriginalPrototype,
  );
  ...
  else console.log('Original and Copy/Clone have different memory address');

  if (coachOriginalPrototype.id === coachCopy.id)
    console.log('Same memory address, the ID has not been cloned');
  else console.log('Different memory address, the ID has been cloned');
  ...
  else console.log('Different memory address, the preference has been cloned');

  if (coachOriginalPrototype.preference.coach === coachCopy.preference.coach)
    console.log(
      'Same memory address, the coach preference has not been cloned',
    );
  else
    console.log(
      'Different memory address, the coach preference has been cloned',
    );
}
